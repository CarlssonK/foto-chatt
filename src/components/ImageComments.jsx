...
function ImageComments({handleToggleImageComments, showComponentBool, imageId}) {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(`http://localhost:3000/api/rooms/${imageId}/comments`, {
      method: "POST",
      headers: {
    ...
    <div className="image-comments-outer-container">
      <div className="image-comments-container">
        <div className="image-comments-inner-container">
          <div onClick={() => handleToggleImageComments(false)} className="ig-exit">
              ...
              { 
                ...
                <div className="ig-img-box">
                  <Slide transitionDuration={200}>
                    {
                      images.map(e => {
                        return <img key={e._id} src={e.url.replace("/upload", "/upload/w_400")} style={{margin: "0 auto"}}></img>
                      })
                    }
                  </Slide>
                </div>
                :
                <div className="ig-img-box" style={{display: "grid", placeItems: "center", backgroundColor: "black"}}>
                  {
                    images.map(e => {
                      return <img key={e._id} src={e.url.replace("/upload", "/upload/w_400")}></img>
                    })
                  }
                </div>
          ...
          <form onSubmit={handleSubmit}>
              <div className="ig-input-box">
                <input onChange={handleInput} ref={inputRef} type="text" style={{color: "white"}} placeholder="Kommentera..." />
                <button><i className="material-icons right">send</i></button>
              </div>
          </form>
